...
public class Percolation {
    ...
    private WeightedQuickUnionUF info;
    ...
    public Percolation(int n) { // create n-by-n grid, with all sites blocked
        ...
        info = new WeightedQuickUnionUF(n*n + 2);
        for (int i = 0; i < n; ++i) {
    ...
    public void open(int row, int col) { // open site (row, col) if it is not open already
        ...
        if (row - 1 > 0) { 
            if (table[row - 1 -1][col -1] == 1) { 
                ...
                info.union((row-1)*count+col-1, (row-1-1)*count + col - 1);
            }
        }
        ...
        if (col - 1 > 0) {
            if (table[row - 1][col - 1 - 1] == 1) { 
                ...
                info.union((row - 1)*count + col - 1, (row - 1)*count + col - 1 - 1);
            }
        }
        if (col - 1 + 1 < count) {
            if (table[row - 1][col + 1 - 1] == 1) { 
